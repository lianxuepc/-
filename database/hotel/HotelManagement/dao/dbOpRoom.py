...
class Room:
        ...
        try:
            for i in re.split(',|，| ', ttrid):
                ...
                self.cursor.execute("insert into checkin_team values(%s,%s,%s,%s,%s,%s,%s)",
                                    (i, ttid, starttime, tendtime, totalPrice, self.staff.sid, tremark))
            self.db.commit()
            return True
        except Exception as e:
            print(e)
    ...
    def reserveToCheckinT(self,ttid,rrid):
        ...
        for rid in re.split(',|，| ', rrid):
            print(rid)
            self.cursor.execute("select * from booking_team where ttid=%s and rid=%s and start_time=%s",
                                (ttid, rid, starttime))
            data = self.cursor.fetchall()
            print(data)
            if data == ():
            ...
            try:
                self.cursor.execute("insert into checkin_team values(%s,%s,%s,%s,%s,%s,%s)",
                                    (rid, ttid, starttime, endtime, totalPrice, self.staff.sid, remark))
                self.cursor.execute("delete from booking_team where ttid=%s and rid=%s and start_time=%s",
                                    (ttid, rid, starttime))
                self.db.commit()
            except Exception as e:
                print(e)

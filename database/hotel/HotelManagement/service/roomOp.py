...
class RoomOp(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.searchNB.clicked.connect(self.showRoomInfo) # 搜索绑定显示房间信息
    ...
    def showRoom(self, rid, rtype, rstorey, rprice, rdesc, rpic, endtime, i, j):
        ...
        pb.setText("立即订购")
        pb.setStyleSheet("background:#ff7f00;border-radius:8px;\n")
        ...
        for i in range(1 + int(length / 3)):
            for j in range(3):
                if k == length:
                    break
                print(k)
                self.showRoom(da[k]['rid'], da[k]['rtype'], da[k]['rstorey'], da[k]['rprice'], da[k]['rdesc'],
                              da[k]['rpic'], rendtime, i, j)
                k = k + 1
        return True
    ...
    def teamCheckin(self):
        ...
        r = Room()
        print(ttid)
        ret = r.teamCheckinDB(tname, ttid, tphone, trid, tendtime, tremark)
        if ret:
            QMessageBox().information(None, "提示", "入住信息登记完成！", QMessageBox.Yes)
    ...
    def reverseToCheckT(self):
        ttid = self.tid_rtc.text()
        rid = self.trid_rtc.text()
        print(ttid, rid)
        r = Room()
        ret = r.reserveToCheckinT(ttid, rid)
        if ret:
            QMessageBox().information(None, "提示", "预约入住完成！", QMessageBox.Yes)
    ...
    def reserveTeam(self):
        ...
        r = Room()
        ret = r.reserveTDB(tname, ttid, tphone, trid, tstarttime, tendtime, tremark)
        if ret:
            QMessageBox().information(None, "提示", "预约信息登记完成！", QMessageBox.Yes)
    ...
    def setBrowerPath(self):
        download_path = QtWidgets.QFileDialog.getExistingDirectory(self, "选择图片路径", "C:\pictures")

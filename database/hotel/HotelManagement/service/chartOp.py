...
class ChartOp(QMainWindow, Ui_MainWindow):
    ...
    def setBrowerPath(self,lineedit):
        download_path = QtWidgets.QFileDialog.getExistingDirectory(self,"选择导出目录","C:\pictures")
    ...
    def toSQLDB(self):
        ...
        os.system("mysqldump -u hotel -p%s hotel > %s/hotel.sql" % (key,path))
    ...
    def toSQLTable(self):
        ...
        if table_name == '请选择...':
            ...
            return False
        os.system("mysqldump -u hotel -p%s hotel %s > %s/%s.sql" %(key,table_name,path,table_name))
